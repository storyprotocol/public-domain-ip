...
class Test(unittest.TestCase):
    ...
    def test_anchor_divider(self):
        ...
        d = AnchorDivider(
            url=test_url, anchor_list=["link1", "link2", "link3", "link4", "link5"]
        )
        chapters = []
        for chapter in d.divide():
            chapters.append(chapter)
        ...
        self.assertTrue(
            chapters[-1]["content"].startswith("Yes! and the bedpost was his own.")
        )
        self.assertTrue(
            "as Tiny Tim observed, God bless Us, Every One!" in chapters[-1]["content"]
        )
    ...
    def test_content_divider(self):
        ...
        self.assertTrue(
            chapters[0]["content"].startswith("Happy families are all alike;")
        )
        self.assertTrue(
        ...
        self.assertTrue(
            chapters[-1]["content"].startswith(
                "Going out of the nursery and being again alone"
            )
        )
        self.assertTrue(
            "which I have the power to put into it.”" in chapters[-1]["content"]
        )

    def test_divider_factory(self):
        ret = divider_factory(
            "1", "https://www.gutenberg.org/cache/epub/1399/pg1399-images.html", ""
        )
        self.assertIsInstance(ret, GenericDivider)

        ret = divider_factory(
            "2", "https://www.gutenberg.org/cache/epub/1399/pg1399-images.html", ""
        )
        self.assertIsInstance(ret, AnchorDivider)

        ret = divider_factory(
            "3", "https://www.gutenberg.org/cache/epub/1399/pg1399-images.html", ""
        )
